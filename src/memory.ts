...
export class MemoryService {
  ...
  async list(repositoryId: number, status?: MemoryRecord["status"]) {
    const result = await this.database.pool.query("SELECT DISTINCT ON (id) * FROM memories WHERE repository_id=$1 ORDER BY id,version DESC", [repositoryId]);
    return result.rows.map(toMemory).filter((memory) => !status || memory.status === status);
  }

  async get(repositoryId: number, id: string) {
    const result = await this.database.pool.query("SELECT * FROM memories WHERE repository_id=$1 AND id=$2 ORDER BY version DESC LIMIT 1", [repositoryId, id]);
  ...
  async edit(repositoryId: number, id: string, patch: CandidatePatch, actor: Actor) {
    ...
    try {
      return await transaction(client, async () => {
        const currentResult = await client.query("SELECT * FROM memories WHERE repository_id=$1 AND id=$2 ORDER BY version DESC LIMIT 1 FOR UPDATE", [repositoryId, id]);
  ...
  async transition(repositoryId: number, id: string, action: "approve" | "reject" | "deprecate", actor: Actor) {
    ...
    try {
      return await transaction(client, async () => {
        const result = await client.query("SELECT * FROM memories WHERE repository_id=$1 AND id=$2 ORDER BY version DESC LIMIT 1 FOR UPDATE", [repositoryId, id]);
        ...
        if (action === "approve" && before.type === "exception") {
          if (!before.exceptionTo || !Object.values(before.scope).some((value) => value?.length)) throw new Error("例外必须关联原规则并限定 Scope");
          const original = await client.query("SELECT 1 FROM memories WHERE repository_id=$1 AND id=$2 AND status='ACTIVE'", [repositoryId, before.exceptionTo]);
  ...
  async supersede(repositoryId: number, oldId: string, candidateId: string, actor: Actor) {
    ...
    try {
      return await transaction(client, async () => {
        const oldRow = await client.query("SELECT * FROM memories WHERE repository_id=$1 AND id=$2 ORDER BY version DESC LIMIT 1 FOR UPDATE", [repositoryId, oldId]);
        const candidateRow = await client.query("SELECT * FROM memories WHERE repository_id=$1 AND id=$2 ORDER BY version DESC LIMIT 1 FOR UPDATE", [repositoryId, candidateId]);
  ...
  async retrieve(repositoryId: number, input: { paths: string[]; text: string; conditions?: string[]; limit?: number }) {
    const memories = (await this.list(repositoryId, "ACTIVE")).filter((memory) => scopeMatches(memory, input.paths, input.conditions ?? []));
