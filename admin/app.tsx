...
type Repo = { id:number; fullName:string; installationId:number; enabled:boolean; includePaths:string[]; excludePaths:string[]; outputLanguage:string; budgetTokens:number; reviewMode:"single"|"auto"; maxDelegates:number };
...
function Jobs({items}:{items:Job[]}) {
  return <section><div className="section-head"><div><p className="eyebrow">EXECUTION</p><h2>Jobs</h2></div><p>查看执行状态、消耗与未完成的检查。</p></div>
    {!items.length?<Empty>暂无任务。</Empty>:<div className="table-wrap"><table><thead><tr><th>任务</th><th>仓库 / PR</th><th>Head</th><th>状态</th><th>尝试</th><th>耗时</th><th>执行详情 / 结果</th></tr></thead><tbody>
      {items.map(job=><tr key={job.id}><td>{job.job_type}<small>{new Date(job.updated_at).toLocaleString()}</small></td><td>{job.repository}<small>PR #{job.pr_number}</small></td><td><code title={job.target_sha}>{String(job.target_sha).slice(0,10)}</code></td><td><Status value={job.status}/></td><td>{job.attempt}</td><td>{job.started_at&&job.finished_at?Math.max(0,new Date(job.finished_at).getTime()-new Date(job.started_at).getTime())+" ms":"—"}</td><td>
        ...
        {job.github_review_url?<a href={job.github_review_url}>Review</a>:job.last_error?<span className="error">{job.last_error}</span>:<a href={"https://github.com/"+job.repository+"/pull/"+job.pr_number}>Source PR</a>}
      </td></tr>)}
    </tbody></table></div>}
  </section>;
}
...
function App(){const [session,setSession]=useState<any>(null);const [page,setPage]=useState("memories");const [repos,setRepos]=useState<Repo[]>([]);const [jobs,setJobs]=useState<Job[]>([]);const [memories,setMemories]=useState<Memory[]>([]);const [findings,setFindings]=useState<any[]>([]);const [error,setError]=useState("");async function load(){try{const s=await api<any>("/api/session");csrf=s.csrf;setSession(s.user);const [r,j,m,f]=await Promise.all([api<Repo[]>("/api/repositories"),api<Job[]>("/api/jobs"),api<Memory[]>("/api/memories"),api<any[]>("/api/findings")]);setRepos(r);setJobs(j);setMemories(m);setFindings(f);setError("")}catch(e){setError(e instanceof Error?e.message:"加载失败")}}useEffect(()=>{void load()},[]);if(!session)return <main className="login"><div className="login-card"><p className="eyebrow">PI REPOSITORY GOVERNANCE</p><h1>把团队决定带进下一次 Review。</h1><p>登录后确认候选规则、管理作用域，并排查审查任务。</p>{error&&<p className="error">{error}</p>}<a className="button" href="/auth/github">Login with GitHub</a></div></main>;return <><header className="top"><div><strong>Team Memory</strong><span>{session.login}</span></div><nav aria-label="主导航">{[["memories","Memory"],["repositories","Repositories"],["jobs","Jobs"],["discussions","Discussions"]].map(([id,label])=><button className={page===id?"active":""} onClick={()=>setPage(id)} key={id}>{label}</button>)}</nav><button className="secondary" onClick={()=>void api("/auth/logout",{method:"POST",body:"{}"}).then(()=>location.reload())}>退出</button></header><main className="shell">{error&&<p className="alert">{error}</p>}{page==="repositories"&&<Repositories items={repos} reload={()=>void load()}/>} {page==="jobs"&&<Jobs items={jobs}/>} {page==="memories"&&<Memories items={memories} repositories={repos} reload={()=>void load()}/>} {page==="discussions"&&<Discussions items={findings}/>}</main></>}
