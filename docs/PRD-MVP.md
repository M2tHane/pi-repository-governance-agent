...
M2：Reply Handler、Main Agent、delegate_agent、专项 SubAgent；技术实现与验收证据已记录，独立人工增益确认仍待完成，具体状态和固定样本增益以 [M2 清单](./tasks/M2.md) 为准。

M3：Health Auditor、定时与手动仓库健康检查、健康报告与历史对比；产品范围见 [F06](#f06仓库健康检查与报告m3)，执行状态见 [M3 清单](./tasks/M3.md)。
...
- PR 顶部只显示问题数量、严重度、位置和短标题，注明静态检查与实际未完成的范围；完整审计进入详情。
- 行内评论按“问题 → 原因 → 修改建议”组织，正文合计最多 120 字符；必要时附简短代码。规则来源默认折叠。
- 同一根因链只发布一个问题，保留主位置、关联位置与完整原始意见；不同团队规则不能被混为一条约束。
- 无法可靠定位到 diff 的问题放入摘要，其简短原因与建议可直接展开查看。
- 团队规则类意见必须绑定实际召回的 id/version，页面显示规则名称与来源。

这些展示要求同时适用于单 Agent 和 Main 汇总；专项只提交审计候选，避免重复生成展示文本。
...
GitHub PR 是开发者主要交互入口。管理页围绕当前待办与团队规则组织信息：

| 入口 | 默认内容与操作 |
| --- | --- |
| 概览 | 待确认规则、运行异常、每个 PR 最近一次审查、实际健康检查结论 |
| 团队规则 | 搜索、仓库与状态筛选；列表只显示名称、范围、来源和状态 |
| 仓库 | 接入状态与启停；细节进入审查策略 |
| 设置 | 审查策略、仓库健康检查、默认筛选失败的运行记录、审查讨论 |

规则快速查看使用右侧抽屉，复杂编辑进入独立页面，停用使用居中确认弹窗。待确认规则提供“忽略 / 编辑 / 确认加入团队规则”；规则更新确认前，旧版继续参与审查。替代规则按名称选择并显示内容预览，不要求填写标识。

来源与证据统一为“这条规则为什么存在”；版本历史、置信度说明和技术信息进入二级入口。UUID、原始 JSON 和内部状态不进入默认阅读路径，不提供没有来源的手工造规则入口。

健康页面先展示已有意见与覆盖缺口，再展开完整报告与历史比较。没有 CI 数据时显示覆盖不足，不推断“健康良好”、规则使用次数或总分。原有报告、运行记录、来源与版本仍可追溯，页面不暴露模型内部思维过程。

体验收口的执行与验证记录见 [UX 清单](./tasks/UX.md)。

### F06｜仓库健康检查与报告（M3）

目标用户是现有已接入仓库的维护者与技术负责人。Health Auditor 帮助用户了解默认分支的当前问题、证据和未检查范围，并与历史观察对照。

核心流程：维护者登录 → 选择仓库 → 手动执行 → 查看队列状态 → 阅读固定快照报告 → 查看历史对比 → 按需启用定时检查。

首版覆盖现有管理页已登记、已授权且已启用的仓库。检查对象是仓库默认分支；服务在创建任务时解析并固定 commit SHA。排队或执行期间默认分支更新，原任务仍检查原快照，报告显示目标 SHA 与采集时间。健康任务不依赖某个 PR。

#### 必须有及验收标准

| 编号 | 能力 | 可观察的完成条件 |
| --- | --- | --- |
| H01 | 维护者手动触发 | 点击后快速得到可重新访问的持久任务；刷新不丢状态；同一仓库已有排队或执行中的健康任务时，重复点击返回该任务。未登录、越权、暂停仓库均不能触发。 |
| H02 | 固定快照与数据窗口 | 任务固定仓库、默认分支名、SHA 和最近 30 天的数据窗口；重试保持这些输入。空仓库或默认分支不存在时明确说明无法创建快照。报告中的代码和引用都能追溯到该 SHA。 |
| H03 | 有限且透明的分析范围 | 沿用仓库包含／排除路径；服务限定文件清单、读取内容、总 Token 和执行时间。同一健康任务跨重试／恢复累计实际与未知消耗，不能重置总额度。报告列出实际检查范围与截断。团队规则只使用当前检索策略实际召回、符合 Scope 的 ACTIVE Memory，记录实际 id/version 与召回限制。 |
| H04 | 多维证据 | 至少分别呈现代码／架构与团队规则、既有 CI／测试结果、依赖或既有扫描线索、文档与实现一致性。每条 finding 有触发条件、影响、证据、建议及独立的严重度／证据强度；没有基线或数据的维度明确标记缺失。 |
| H05 | 可核验的 CI 数据 | 只读取已有权限允许访问的记录。目标 SHA 对应的 CI 与窗口内默认分支历史记录分开展示，并保留各记录的 SHA、时间和来源链接。权限不足、无记录、SHA 不匹配、窗口外和采集截断分别标记；无数据不推导为通过。 |
| H06 | 持久报告与失败语义 | 管理页能重开历史报告，展示快照、窗口、采集时间、findings、coverage、missingData、limitations、模型、usage 和耗时。部分来源缺失仍可保存有效分析，并显著标记 partial；未建立快照、结果无效或无可校验分析结果时为 failed；取消为独立状态。 |
| H07 | 明确启用的定时检查 | 默认关闭；维护者可选择每日或每周，启用时默认建议每周。首次执行在启用后的一个周期到期，调度统一按 UTC 计时，页面展示时区和下次执行时间；修改频率从修改时重新计算。停机恢复最多补一个到期任务，不密集重放历史周期；与手动触发共享活动任务去重。 |
| H08 | 受控队列与取消 | 同一仓库最多一个活动健康任务，复用单 Worker，待领取的 PR／Reply／Decision 任务优先于健康任务；不抢占正在运行的健康任务。GitHub 读取、Workspace 准备、文件读取和模型调用均受任务总时限与取消约束。关闭定时只停止后续定时触发；暂停仓库、撤销授权或停止服务须中止相关活动分析，并阻止其形成新的有效报告；保留此前已完成报告。 |
| H09 | 历史与最小趋势 | 展示报告列表及最近两份可比较报告的各维度观察数量、严重度分布和覆盖差异。对比要求同仓库、相同路径范围／采样策略／窗口长度／模型与检查版本／规则版本，且相关维度的数据完整度可比。覆盖不足时显式标注不可直接比较；数量下降不自动认定问题已修复。首版只做指标与证据对照，逐问题归因交给维护者。 |
| H10 | 权限与只读边界 | 报告读取、触发和配置沿用维护者 OAuth／CSRF 与仓库授权检查；GitHub 读取仍使用 App installation token。Agent 只获得服务绑定的只读上下文和工具，不能选择仓库、SHA、cwd、凭据或工具集，也不能修改源码或 ACTIVE Memory。 |

各维度的解释边界：存在测试文件不等于测试通过或覆盖充分；读取依赖清单不等于完成漏洞扫描；架构规则需要实际适用的基线。外部数据缺失不能由模型猜测补齐。零 findings 应表述为“在所列范围内未发现问题”。

报告首先保存在服务与现有管理页。保留当前管理页的视觉风格，补充 Health 入口、运行状态、报告详情和对比表；不要求新的健康总分或图表系统。首版 Health Auditor 使用独立会话与现有请求预算保护，预算不足在下一次请求前拦截；没有 usage 回报的消耗保留为未知预留。

#### 空、等待与异常状态

- 没有报告：说明检查用途，并为有权限的维护者提供手动执行入口。
- 排队／执行中：显示固定目标和时间，不伪造完成百分比；已完成的历史报告仍可查看。
- 只有一份报告：明确“暂无可比较报告”；部分覆盖的维度不进入正常趋势判断。
- OAuth 过期或无权限：提供重新登录或说明权限不足，不返回其他仓库数据。
- 预算、超时、采集或模型失败：显示具体失败阶段、已完成范围和可用的重试入口；自动重试仍遵循现有有限重试策略。

#### 后续再做与明确不做

后续按使用证据再考虑：未登记安装仓库的全量同步、多分支／自选 ref、自定义数据窗口、任意 cron、通知／导出、报告保留策略、跨报告问题身份、完整安全扫描接入和更多 Agent 委派。M3 不改变现有 Memory 生命周期和检索版本语义。

首版不执行待审仓库的 hooks、安装、构建、测试或扫描脚本；不自动创建 GitHub 评论、Issue 或修复 PR，不申请新的 Actions／Checks／Security alerts 权限。Repository Curator、代码清理、源码修改、自动修复／commit／push／merge 和多 Worker 均不在本阶段范围内。
