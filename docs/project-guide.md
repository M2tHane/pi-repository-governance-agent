...
建议先读第 1～4 节建立整体印象，再读数据库和 Pi 细节；遇到英文状态时查第 10 节。启动配置见 [README](../README.md)，此前四个问题的回答见 [运行流程问答](runtime-flow.md)。
...
当前设计不把“代码刚好这样写”直接提升为团队规则，也不把 AI 自己的建议或尚未解决的争议当作已确认决定。
...
5. 有仓库管理权限的维护者在管理页确认，规则成为 `ACTIVE`（生效）。
6. 后续 PR 可能召回这条规则。
...
启动时服务执行迁移：按文件顺序检查升级记录，以数据库锁避免迁移冲突，每个迁移在事务内执行并记录。后续结构调整应增加迁移，不改写已经执行过的历史脚本。
...
| 被识别为暂时性网络／限流错误 | 持久化重试时间，有限次数重试 | 不是所有错误都能自动重试；不会无限循环 |
