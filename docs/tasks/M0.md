...
本次项目目录与协作规范的初始化不计入以下业务任务。勾选框仅在任务满足验收并填写真实证据后更新。
...
当前长期设计理由见 [仓库架构决策](../../.agents/decisions/repository-architecture.md) 与 [Workspace 安全决策](../../.agents/decisions/github-workspace-security.md)。
...
2026-09-10 已同步本文件最终状态和真实证据，README 已更新为实际运行方式。当前长期设计理由已归并至 [仓库架构决策](../../.agents/decisions/repository-architecture.md)。
